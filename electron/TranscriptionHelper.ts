...
export class TranscriptionHelper implements ITranscriptionHelper {
  ...
  private isSpeechRecognitionSupported(provider: APIProvider): boolean {
    // OpenAI (Whisper), Azure-OpenAI (Whisper), and Gemini (Audio Understanding) support speech recognition
  ...
  public async transcribeAudio(
    ...
    if (config.apiProvider === "openai" || config.apiProvider === "azure-openai") {
  ...
  private async transcribeWithOpenAI(
    ...
    if (!this.providerClient || !isOpenAICompatible(this.providerClient)) {
      throw new Error('OpenAI-compatible client not initialized.');
    }
    const openai = this.providerClient.client;
  ...
  private async transcribeWithGemini(
    ...
    if (!this.providerClient || this.providerClient.type !== "gemini") {
    ...
    try {
      ...
      const response = await axios.default.post(
        `https://generativelanguage.googleapis.com/v1beta/models/${speechModel}:generateContent?key=${geminiApiKey}`,
        {
  ...
  private async transcribeWithAnthropic(
    ...
    if (!this.providerClient || this.providerClient.type !== "anthropic") {
