...
const state = {
  ...
  modelFetchService: null as import('./ModelFetchService').ModelFetchService | null,
...
export interface IIpcHandlerDeps {
  ...
  modelFetchService?: import('./ModelFetchService').ModelFetchService
}
...
async function initializeHelpers() {
  ...
  const { ModelFetchService } = await import('./ModelFetchService')
  state.modelFetchService = new ModelFetchService()
...
async function initializeApp() {
  try {
    ...
    const appDataPath = path.join(app.getPath('appData'), 'interview-assistant-v1')
    ...
    initializeIpcHandlers({
      ...
      moveWindowUp: () => moveWindowVertical((y) => y - state.step),
      moveWindowDown: () => moveWindowVertical((y) => y + state.step),
      transcriptionHelper: state.transcriptionHelper || undefined,
      conversationManager: state.conversationManager || undefined,
      answerAssistant: state.answerAssistant || undefined,
      modelFetchService: state.modelFetchService || undefined
    })
    await createWindow()
