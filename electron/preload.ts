...
const electronAPI = {
  ...
  updateConfig: (config: { apiKey?: string; model?: string; language?: string; opacity?: number; apiProvider?: string; extractionModel?: string; solutionModel?: string; debuggingModel?: string; answerModel?: string; speechRecognitionModel?: string; candidateProfile?: any; azureEndpoint?: string; azureApiVersion?: string }) =>
    ipcRenderer.invoke("update-config", config),
  fetchProviderModels: (provider: string, apiKey: string, options?: { endpoint?: string; apiVersion?: string; forceRefresh?: boolean }) =>
    ipcRenderer.invoke("fetch-provider-models", provider, apiKey, options),
