...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  // Dynamic model fetching handler
  ipcMain.handle("fetch-provider-models", async (_event, provider: string, apiKey: string, options?: { endpoint?: string; apiVersion?: string; forceRefresh?: boolean }) => {
    try {
      if (!deps.modelFetchService) {
        return { success: false, models: [], error: "Model fetch service not initialized" };
      }
      const result = await deps.modelFetchService.fetchModels(
      ...
      return { success: false, models: [], error: error.message || "Failed to fetch models" };
    }
  })
